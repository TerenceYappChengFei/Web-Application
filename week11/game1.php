...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $limit = "SELECT attempts_g1 FROM uid WHERE UID = '$uid'";
    $result = mysqli_query($conn, $limit);
    $row = mysqli_fetch_assoc($result);
    $attempts = $row['attempts_g1'];   
    $attempts++;
    
    echo "Attempts: " . $attempts . "<br>";

    if($attempts == 2) {
        $message = "You have reached the limit of 2 attempts for Game 1.";
    }
    else if($attempts < 2) {
     $update = "UPDATE uid
               SET G1 = '$game1', attempts_g1 = '$attempts'
               WHERE UID = '$uid'";
              //  $_POST['attempts']++;
